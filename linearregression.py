...
def predict_y(data, path):
        ...
        for i in list(data2['Subgroup#'].unique()):
            for r in list(data2.Round.unique()):
                for s in list(data2['SC:BC_Ratio_Bucket'].unique()):
                    ...
                    conditions = (data2['Subgroup#']==i) & (data2['Round']==r) & (data2['SC:BC_P4_Ratio_Bucket']==s)
        ...
        data.loc[(data['BC_Progesterone (pg/mL)_filled']>1500), 'BC_TRIGGER_f'] = 1
        ...
        data.loc[(data['BC_Estradiol (pg/mL)_filled']>1500), 'BC_E_TRIGGER_f'] = 1
