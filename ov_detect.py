...
class detection():
    ...
    def baseline(self, player, cycle, column, base):
        ...
        basel = indata[column].iloc[0:base].mean()
    ...
    def p4_ov_detect(self, data,  start_threshold):
        ...
        if len(indxlist)>4:
            threshold = indata.loc[indata['Day'] == start_threshold].index[0]
            #if len(countback)>0:
    ...
    def p4_ov_day(self, data):
        ...
        for j in range(i, i+(len(indata))-17):
    ...
    def plotcycle(self, data, org, p, c):
        ...
        plt.title('Player '+str(p)+' Cycle '+str(c), fontsize = 16)
        
        
        # plt.grid(True, axis = 'x' )
        #ax1.grid(True, which = 'both', axis = 'x', color='#DDDDDD', linestyle='--', linewidth=1 )
        plt.savefig(self.path + '/'+p+'/'+c+'/{}_{}_{}.png'.format(org, p, c), format='png', dpi=1200, bbox_inches='tight', facecolor='w', transparent=False)
        plt.savefig(self.path + '/{}_{}_{}.png'.format(org, p, c), format='png', dpi=1200, bbox_inches='tight', facecolor='w', transparent=False)
        plt.show()

    def plotcycle_no_ov(self, data, org, p, c):
        ...
        # plt.grid(True, axis = 'x' )
        #ax1.grid(True, which = 'both', axis = 'x', color='#DDDDDD', linestyle='--', linewidth=1 )
        plt.savefig(self.path + '/'+'Without_ovulation_markers'+'/{}_{}_{}.png'.format(org, p, c), format='png', dpi=1200, bbox_inches='tight', facecolor='w', transparent=False)
        plt.show()
