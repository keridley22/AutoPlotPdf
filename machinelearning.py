...
class classifier():
    def __init__(self, dataframe_1, dataframe_2, path):
...
'BC_E2_Decision':[], 'BC_P4_Decision':[], 'BC_E2_Decision_f':[], 'BC_P4_Decision_f':[], 'BC_E2_Decision_1000':[], 'BC_P4_Decision_1000':[]}
        ...
        for p in patients:
            ...
            data['BC_TRIGGER'].replace(np.nan, 0, inplace=True)
        ...
        for p in list(data2['ID'].unique()):
            ...
            data.loc[(data['ID']==p), 'SF_Time']=data3.loc[(data3['Patient ID']==p) & (data3['SampleType'] == 'Saliva Fasting'), 'TimeH'].astype(float).mean()
            data.loc[(data['ID']==p), 'SC_Time']=data3.loc[(data3['Patient ID']==p) & (data3['SampleType'] == 'Saliva Clinic'), 'TimeH'].astype(float).mean()
            BCS1=((data3['SampleType'] == 'Blood Clinic') & (data3['Round'] == 1.0))
            ...
            data.loc[(data['ID']==p), 'SF_E2_quartile']=data2.loc[(data2['ID']==p), 'SF_Estradiol (pg/mL)_quartiles'].mean()
            data.loc[(data['ID']==p), 'SC_E2_quartile']=data2.loc[(data2['ID']==p), 'SC_Estradiol (pg/mL)_quartiles'].mean()
            data.loc[(data['ID']==p), 'SF_P4_quartile']=data2.loc[(data2['ID']==p), 'SF_Progesterone (pg/mL)_quartiles'].mean()
            data.loc[(data['ID']==p), 'SC_P4_quartile']=data2.loc[(data2['ID']==p), 'SC_Progesterone (pg/mL)_quartiles'].mean()
            ...
            data['SF_Time']=data['SF_Time'].fillna(data['SF_Time'].mean())
            data['SC_Time']=data['SC_Time'].fillna(data['SC_Time'].mean())
    ...
    def fit_rf_grid(self, data):  
        ...
        xdata=data.drop(['BC_E2_Decision', 'BC_P4_Decision', 'BC_P4_Decision_1000', 'BC_E2_Decision_1000', 'BC_P4_Decision_f', 'BC_E2_Decision_f', 'Subgroup', 'ID'], axis=1)
        ...
        for xo in x_opts:
            for target in ys:
                ...
                joblib.dump(classifier, path+"/rf_{}_{}.joblib".format(target.split('_')[1], xo[0][:2]))
    ...
    def test_rf_model(self, data):
        ...
        rf_dict = {'target': [], 'X variables':[],'cross_val_#':[], 'accuracy': [], 'precision': [], 'recall': [], 'specificity': [], 'cross_val_accuracy_mean': [], 
        ...
        for xo in x_opts:
            ...
            for target in ys:
                ...
                classifier = joblib.load(path+"/rf_{}_{}.joblib".format(target.split('_')[1], xo[0][:2]))
                #print(classifier)

                y_pred = classifier.predict(X_test)
                #print(y_pred)
                print(target.split('_')[1], xo[0][:2])
                ...
                plt.savefig(path+'confusion_matrix_{}_{}.png'.format(target.split('_')[1], xo[0][:2]), dpi=300)
                ...
                ax.set_title('Feature importance: {} Decision with {} X variables'.format(target.split('_')[1], xo[0][:2]))
                #ax.set_title("Feature importances using MDI")
                #ax.set_ylabel("Mean decrease in impurity")
                #fig.tight_layout()
                plt.tight_layout()
                plt.savefig(path+'feature_importance_{}_{}.png'.format(target.split('_')[1], xo[0][:2]), dpi=1200)
                plt.show()
                ...
                for i in range(0, 5):
                    ...
                    rf_dict['X variables'].append(xo[0][:2])
                ...
                doc.add_heading('Predicting {} trigger decisions with X variables from {} measurements'.format(target.split('_')[1], xo[0][:2]), level=1)
                doc.add_paragraph('Accuracy: {}'.format(metrics.accuracy_score(y_test, y_pred)))
                doc.add_paragraph('Precision: {}'.format(metrics.precision_score(y_test, y_pred, zero_division=1)))
                doc.add_paragraph('Recall: {}'.format(metrics.recall_score(y_test, y_pred, zero_division=1)))
                doc.add_paragraph('Specificity: {}'.format(specificity))
                doc.add_paragraph(' ')
                ...
                doc.add_picture(path+'confusion_matrix_{}_{}.png'.format(target.split('_')[1], xo[0][:2]), width=Inches(5))
                doc.add_paragraph(' ')
                ...
                doc.add_picture(path+'feature_importance_{}_{}.png'.format(target.split('_')[1], xo[0][:2]), width=Inches(5))
                doc.add_paragraph(' ')
        ...
        rf_df.to_csv(path+'random_forest_results.csv')
        #print(rf_df)
        ...
        doc.save(path + 'random_forest_results.docx')
        convert(path+'random_forest_results.docx', path+'random_forest_results.pdf')
